...
def inicio():
    ...
    if request.method == "POST":
        if 'archivo_codigo' in request.files and request.files['archivo_codigo'].filename != '':
            ...
            # ARREGLO #2: antes era solo  instruccion_texto = file.read().decode('utf-8').strip()
            # y eso reventaba (Error 500) si el .txt traía tildes guardadas en otra codificación
            # (ej. Bloc de notas en ANSI/Windows-1252). Ahora intenta UTF-8 y si falla usa latin-1.
            try:
                instruccion_texto = datos_binarios.decode('utf-8').strip()
            except UnicodeDecodeError:
                instruccion_texto = datos_binarios.decode('latin-1').strip()
        else:
        ...
        if instruccion_texto:
            ...
            # ARREGLO #1 (EL IMPORTANTE): antes decía
            #     if "válido" in resultado.lower():
            # pero analizar_sintaxis() devuelve el texto "Ficha Válida" (con A, femenino),
            # y la palabra "válido" (con O) NUNCA es substring de "válida". Por eso esta
            # condición era SIEMPRE False, sin importar qué tan perfecta estuviera la ficha.
            # Se reemplaza por el mismo criterio que ya usa tu propio index.html:
            # "si no contiene la palabra Error, fue exitoso".
