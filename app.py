...
def inicio():
    ...
    if request.method == "POST":
        ...
        if instruccion_texto:
            ...
            # CORRECCIÓN #1: Filtro robusto que verifica si no hay errores en el resultado
            if "Error" not in resultado:
                ...
                if "Error" not in resultado:
                    ...
                    session['ultima_traduccion'] = (
                        ...
                        f"1. ANALISIS LEXICO:\n{session.get('detalle_lexico', '')}\n\n"
                        f"2. ANALISIS SINTACTICO:\n{session.get('detalle_sintactico', '')}\n\n"
                        f"3. TRADUCCION TRADUCIDA (JSON):\n{session.get('traduccion_json', '')}"
                    )
            else:
